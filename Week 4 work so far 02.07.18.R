...
# Try gather first.This works but I have lost the row index of 1:7352 
# that is necessary for bringing in the positions

Train_Stats_1 <- Train_Stats
colnames(Train_Stats_1) <- c(1:561)
Train_Stats_2 <- gather(Train_Stats_1,Stat_Names_Index,Stat_Calc,1:561)
#This works. Get rid of column
Train_Stats_3 <-Train_Stats_2
Train_Stats_3[,1] <-NULL

# so can I get rid of NA column?

        
# Making Stat_Names into something I can use
tail(Stat_Names) # this is still giving 561 obs V2: Factor w/ 477 levels
str(Stat_Names)
Stat_Names[475:561,]
c(as.character.factor(Stat_Names$V2))
str(Train_Stats)
Train_Stats$

#Assign column names
colnames(Train_Stats) <- c(as.character.factor(Stat_Names$V2)) # this works
...
# Merge Position with Train_Activity

Merge_1 <- merge(Train_Activity, Position, "V1")
colnames(Merge_1) <- c("Position_number", "Index", "Position")
Merge_2 <- merge(Merge_1,Train_Stats, "Index")
Merge_3 <- merge(Merge_2,Train_Subjects, "Index")
# now you have some extra columns NA.x, NA.1, NA.y, Position_number 

#Try gather

# Example Train_Stats_2 <- gather(Train_Stats_1,Stat_Names_Index,Stat_Calc,1:561)


Gather_1 <- gather(Merge_3,Stat_Names_Index,Stat_Calc,,)
